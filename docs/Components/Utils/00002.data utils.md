...
Instead, it acts as a clean, well-tested utility layer with functions that are called from multiple notebooks/scripts. Typical responsibilities include:
...
3. **Basic Data Validation & Quality Checks**
   - `validate_metrics_df(df)` → check for expected columns, date range, negative values, extreme outliers, missing server_ids
   - `detect_anomalies_simple(df, z_threshold=4)` → basic z-score flagging (mirrors your Citi scipy usage)
